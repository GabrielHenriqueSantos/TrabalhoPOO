...
public class PredioResidencialCreator implements ImovelCreator{
    @Override
    public Imovel_CleitonErinaGabriel criar(Map<String, Object> data) {
        String endereco = (String) data.get("endereco");
        LocalDate dataConstrucao = (LocalDate) data.get("dataConstrucao");
        ...
        int andar = (int) data.get("andar");
        int numApto = (int) data.get("numApto");
        float valorCondominio = (float) data.get("valorCondominio");

        return new PredioResidencial_CleitonErinaGabriel(
                endereco, dataConstrucao,
                areaTotal, areaConstruida, qtdDormitorios, qtdBanheiros,
                qtdVagasGaragem, valorIPTU, valorVenda, valorAluguel,
                tipoOperacao,andar,numApto,valorCondominio
        );
    }
}
