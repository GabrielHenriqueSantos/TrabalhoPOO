...
public class FormCorretor extends GridPane {
    ...
    public FormCorretor(CorretorController controller) {
        ...
        this.add(new Label("CRECI"), 0,8);      this.add(txtCreci,1,8);
        this.add(new Label("Salario"),0,9);     this.add(txtSalario,1,9);
        this.add(new Label("PIS"),0,10);        this.add(txtPis,1,10);
        this.add(new Label("Data de Admissão"),0,11); this.add(dpDataAdmissao,1,11);
        ...
        btnSalvar.setOnAction(e -> {
            try {
                ...
                boolean sucesso = controller.cadastrarCorretor(cpf, nome, rg, dataNasc, endereco, cep, telefone, email,creci,salario,pis,dataAdm);

                Alert alert = new Alert(sucesso ? Alert.AlertType.INFORMATION : Alert.AlertType.ERROR);
                alert.setTitle("Cadastro de Corretor");
                alert.setHeaderText(null);
                alert.setContentText(sucesso ? "Corretor cadastrado com sucesso!" : "Erro ao cadastrar corretor.");
                alert.showAndWait();
